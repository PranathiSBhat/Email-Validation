#!/usr/bin/env python3
"""
Setup script to populate the spam detection dashboard with sample data
"""
...
def setup_dashboard():
    """Setup the dashboard with sample data"""
    ...
    try:
        ...
        for text in spam_texts:
            cursor.execute(
                ...
                (text, 1, datetime.now() - timedelta(hours=random.randint(1, 48)))
            )
        ...
        for text in ham_texts:
            cursor.execute(
                "INSERT INTO spam_results (email_text, xgboost_pred, created_at) VALUES (%s, %s, %s)",
                (text, 0, datetime.now() - timedelta(hours=random.randint(1, 48)))
            )
        ...
        print("=" * 50)
        print("📱 Access the dashboard at: http://127.0.0.1:5000/spam-dashboard")
...
if __name__ == "__main__":
    print("🚀 Setting up Spam Detection Dashboard...")
    print("=" * 50)
    setup_dashboard()